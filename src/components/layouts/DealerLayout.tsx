  ...
  return (
    <SimpleModeContext.Provider value={{ simple: simpleMode, toggle: toggleSimple }}>
      ...
      <div className="min-h-screen flex bg-background">
        {/* Sidebar */}
        <aside className={`${collapsed ? "w-16" : "w-60"} flex-shrink-0 border-r border-border/60 bg-card/80 backdrop-blur-sm flex flex-col transition-all duration-300 hidden md:flex relative`}>
          {/* Subtle corner glow */}
          ...
          <div className="p-3 flex items-center justify-between border-b border-border/60">
            ...
            <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={() => setCollapsed(!collapsed)}>
          ...
          {/* Simple Mode Toggle */}
          {!collapsed && (
            <button onClick={toggleSimple} className="mx-3 mt-3 mb-1 flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs transition-all hover:bg-secondary/60">
              {simpleMode ? <ToggleRight className="w-4 h-4 text-primary" /> : <ToggleLeft className="w-4 h-4 text-muted-foreground" />}
              <span className={simpleMode ? "text-primary font-medium" : "text-muted-foreground"}>Simple Mode</span>
            ...
            {navGroups.map(group => {
              ...
              return (
                <div key={group.label} className="mb-3">
                  {!collapsed && (
                    <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground/60 px-3 pt-3 pb-1.5">
                      {group.label}
                    </p>
                  )}
          ...
          <div className="p-2.5 border-t border-border/60">
            ...
            <Button variant="ghost" size="sm" className="w-full justify-start text-muted-foreground hover:text-foreground/70 text-xs" onClick={handleLogout}>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="md:hidden h-14 border-b border-border/60 flex items-center px-4 gap-3 bg-card">
            <Logo className="h-5" />
          ...
          {/* Mobile nav */}
          <div className="md:hidden flex overflow-x-auto border-b border-border/60 bg-card px-2 scrollbar-hide">
          ...
          {/* Desktop top bar */}
          <div className="hidden md:flex items-center justify-between px-6 py-2 border-b border-border/60 bg-card/50">
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" variant="outline" className="rounded-lg h-8 text-xs px-4 border-border text-muted-foreground bg-transparent hover:bg-secondary/60" onClick={() => navigate("/dealer/claims")}>
                <ClipboardList className="w-3.5 h-3.5 mr-1" /> New Claim
              </Button>
            </div>
            <div className="relative" ref={notifRef}>
              <Button variant="ghost" size="icon" className="relative h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => setShowNotifs(!showNotifs)}>
                <Bell className="w-4 h-4" />
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-destructive text-destructive-foreground text-[10px] flex items-center justify-center font-bold">{unreadCount}</span>
                )}
...
function NotificationDropdown({ notifications, notifStore, userId, navigate, onClose, notifIcon }: any) {
  return (
    <div className="absolute right-0 top-full mt-2 w-80 max-h-96 overflow-auto rounded-xl border border-border bg-popover shadow-xl z-50 animate-fade-in">
        ...
        <div className="divide-y divide-border/40">
          {notifications.slice(0, 15).map((n: any) => {
            ...
            return (
              <button
                key={n.id}
                className={`w-full text-left px-4 py-3 hover:bg-secondary/60 transition-colors flex items-start gap-3 ${!n.read ? "bg-primary/[0.04]" : ""}`}
                onClick={() => {
              ...
              >
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${!n.read ? "bg-primary/15" : "bg-secondary/50"}`}>
