 ...
 */
function MockFrame({ children, gradient = "from-primary/30 via-primary/10 to-transparent" }: { children: React.ReactNode; gradient?: string }) {
  return (
    <div className={`relative rounded-2xl p-3 bg-gradient-to-br ${gradient}`}>
      <div className="rounded-xl overflow-hidden bg-white shadow-2xl ring-1 ring-slate-900/10">
        <div className="flex items-center gap-1.5 px-3 py-2 border-b border-slate-200 bg-slate-50">
          <div className="w-2.5 h-2.5 rounded-full bg-[#ff5f57]" />
          <div className="w-2.5 h-2.5 rounded-full bg-[#febc2e]" />
          <div className="w-2.5 h-2.5 rounded-full bg-[#28c840]" />
          <div className="ml-3 text-[10px] text-slate-400 font-mono">warrantyvault.co.uk</div>
        </div>
        <div className="p-4 sm:p-5 text-slate-900 bg-white">
          {children}
        </div>
      </div>
    </div>
...
function StatBlock({ icon: Icon, label, value, accent = "text-[hsl(172,66%,40%)]" }: any) {
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
      ...
      <div className="text-lg font-bold text-slate-900 font-display">{value}</div>
    </div>
  );
}
...
export function WarrantyManagementMock() {
  const rows = [
    { reg: "BD22 KXR", make: "BMW 320d", customer: "Mark Davies", status: "Active", days: 287, color: "bg-emerald-100 text-emerald-700" },
    { reg: "LK19 NHE", make: "Audi A4", customer: "Sarah Patel", status: "Active", days: 154, color: "bg-emerald-100 text-emerald-700" },
    { reg: "WP21 MTV", make: "VW Golf", customer: "James O'Connor", status: "Expiring", days: 12, color: "bg-amber-100 text-amber-700" },
    { reg: "GH20 RPL", make: "Ford Kuga", customer: "Emma Wilson", status: "Active", days: 198, color: "bg-emerald-100 text-emerald-700" },
    { reg: "RX18 BNT", make: "Mercedes A-Class", customer: "Daniel Hughes", status: "Claimed", days: 0, color: "bg-blue-100 text-blue-700" },
  ];
  return (
    <MockFrame gradient="from-emerald-400/30 via-primary/10 to-transparent">
      ...
      <div className="rounded-lg border border-slate-200 overflow-hidden">
        <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-slate-50 text-[9px] uppercase tracking-wider text-slate-500">
        ...
        {rows.map((r, i) => (
          <div key={i} className="grid grid-cols-12 gap-2 px-3 py-2.5 text-[11px] border-t border-slate-100 hover:bg-slate-50">
            <div className="col-span-3">
              <div className="font-mono font-semibold text-slate-900">{r.reg}</div>
              <div className="text-slate-500 text-[10px]">{r.make}</div>
            </div>
            <div className="col-span-3 text-slate-700 self-center">{r.customer}</div>
            <div className="col-span-3 self-center">
              <span className={`px-2 py-0.5 rounded-full text-[9px] font-semibold ${r.color}`}>{r.status}</span>
            </div>
            <div className="col-span-3 text-right self-center text-slate-600">{r.days > 0 ? `${r.days} days` : "—"}</div>
          </div>
        ))}
      </div>
    </MockFrame>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 mb-3">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-slate-600">Audi A4 2.0 TDI · 2019</p>
          </div>
          <div className="relative w-16 h-16">
            <svg className="w-16 h-16 -rotate-90">
              <circle cx="32" cy="32" r="26" stroke="currentColor" strokeWidth="4" fill="none" className="text-slate-200" />
...
export function ClaimsManagementMock() {
  const claims = [
    { ref: "CLM-2841", customer: "Mark Davies", issue: "Turbocharger failure", status: "Open", color: "bg-amber-100 text-amber-700", priority: "High" },
    { ref: "CLM-2840", customer: "Emma Wilson", issue: "Electrical fault", status: "Approved", color: "bg-emerald-100 text-emerald-700", priority: "Med" },
    { ref: "CLM-2839", customer: "Daniel Hughes", issue: "AC compressor", status: "Pending Info", color: "bg-blue-100 text-blue-700", priority: "Low" },
    { ref: "CLM-2838", customer: "Sarah Patel", issue: "Gearbox slipping", status: "Approved", color: "bg-emerald-100 text-emerald-700", priority: "High" },
  ];
  return (
    <MockFrame gradient="from-amber-400/30 via-primary/10 to-transparent">
      ...
      <div className="space-y-2">
        {claims.map(c => (
          <div key={c.ref} className="rounded-lg border border-slate-200 bg-slate-50 p-3 flex items-center gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="text-[9px] text-slate-400">· {c.priority}</span>
              </div>
              <p className="text-xs font-medium truncate text-slate-900">{c.issue}</p>
  ...
  return (
    <MockFrame gradient="from-purple-400/30 via-primary/10 to-transparent">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="px-2 py-0.5 rounded-full text-[9px] font-semibold bg-amber-100 text-amber-700">Triage: Investigate</span>
      </div>
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 mb-3">
        <div className="flex items-start gap-2">
          ...
          <div className="text-[11px]">
            <p className="font-semibold text-amber-800 mb-0.5">Triage Engine</p>
            <p className="text-slate-700 leading-relaxed">Turbocharger failure at 14k miles. Within CRA 6-month window. Recommend: request invoice + diagnostic report before decision.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-md border border-slate-200 bg-white py-2 text-center text-[10px] text-slate-700">Request Info</div>
        <div className="rounded-md border border-red-200 bg-red-50 py-2 text-center text-[10px] text-red-700">Reject</div>
    ...
    <MockFrame gradient="from-purple-400/30 via-primary/15 to-transparent">
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 mb-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[10px] font-semibold text-amber-800 uppercase tracking-wider">Medium Risk · CRA 6-Month Window</span>
        </div>
        <p className="text-[11px] text-slate-700 leading-relaxed">Customer purchased 4 months ago. Burden of proof sits with the dealer for inherent faults. Repair offered before refund is reasonable.</p>
      </div>
      <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-2">Suggested Response · Helpful tone</p>
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-700 leading-relaxed mb-3">
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {["Helpful", "Firm", "Defensive", "De-escalate"].map((t, i) => (
          <div key={t} className={`text-center text-[9px] py-1.5 rounded-md border ${i === 0 ? "border-[hsl(172,66%,40%)] bg-[hsl(172,66%,40%)]/10 text-[hsl(172,66%,32%)] font-semibold" : "border-slate-200 bg-white text-slate-500"}`}>
            {t}
          </div>
        ))}
      </div>
    ...
    <MockFrame gradient="from-emerald-400/30 via-primary/15 to-transparent">
      ...
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 mb-3">
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] uppercase tracking-wider text-slate-600">Health Score</p>
          <span className="px-2 py-0.5 rounded-full text-[9px] font-semibold bg-emerald-100 text-emerald-700">HEALTHY</span>
        </div>
        <div className="flex items-end justify-between">
          <div>
            <p className="text-3xl font-bold font-display text-slate-900">82</p>
            <p className="text-[10px] text-slate-500">/ 100</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-slate-500">Balance</p>
            <p className="text-lg font-bold font-mono text-slate-900">£8,900</p>
          </div>
        </div>
        <div className="mt-3 h-1.5 rounded-full bg-white overflow-hidden border border-emerald-100">
      ...
      <div className="rounded-lg border border-[hsl(172,66%,40%)]/20 bg-[hsl(172,66%,40%)]/5 p-3">
        ...
        <p className="text-[11px] text-slate-700">Increase contribution to £140/warranty to maintain 2.5x buffer ratio.</p>
      </div>
    </MockFrame>
  );
}
  ...
  return (
    <MockFrame gradient="from-blue-400/30 via-primary/15 to-transparent">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto px-2 py-0.5 rounded-full text-[9px] font-semibold bg-emerald-100 text-emerald-700">ACTIVE</span>
      </div>
      <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 mb-3 text-center">
        <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Your dedicated number</p>
        <p className="text-2xl font-bold font-mono text-slate-900 mb-1">0330 808 4127</p>
      ...
      <div className="space-y-2">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <p className="text-[10px] text-slate-600 italic">"Thanks for calling ABC Automotive warranty support. Press 1 to make a claim, 2 for cover questions, or 3 for general help."</p>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-lg border border-slate-200 bg-white p-2.5 text-center">
            ...
            <p className="text-[9px] text-slate-600">Hold Music</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-2.5 text-center">
            <Voicemail className="w-3.5 h-3.5 text-slate-500 mx-auto mb-1" />
            <p className="text-[9px] text-slate-600">Voicemail</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-2.5 text-center">
            <ChevronRight className="w-3.5 h-3.5 text-slate-500 mx-auto mb-1" />
            <p className="text-[9px] text-slate-600">IVR Menu</p>
          </div>
        </div>
      </div>
    </MockFrame>
...
export function CoverTemplatesMock() {
  const tiers = [
    { name: "Bronze", price: "£89", bg: "bg-amber-50", border: "border-amber-200", text: "text-amber-800", items: 12 },
    { name: "Silver", price: "£149", bg: "bg-slate-100", border: "border-slate-300", text: "text-slate-700", items: 24 },
    { name: "Gold", price: "£249", bg: "bg-yellow-50", border: "border-yellow-300", text: "text-yellow-800", items: 38 },
  ];
  return (
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {tiers.map(t => (
          <div key={t.name} className={`rounded-xl border ${t.border} ${t.bg} p-3`}>
            ...
            <p className="text-base font-bold font-display mb-2 text-slate-900">{t.price}</p>
            <div className="space-y-1">
              {[1,2,3].map(i => (
                <div key={i} className="flex items-center gap-1">
                  <CheckCircle2 className="w-2.5 h-2.5 text-[hsl(172,66%,40%)]" />
                  <div className="h-1 flex-1 rounded-full bg-white/70 border border-slate-200" />
                </div>
              ))}
            </div>
...
export function DocumentsMock() {
  const docs = [
    { name: "Warranty Certificate", type: "PDF", size: "248 KB", color: "bg-red-100 text-red-700" },
    { name: "Terms & Conditions", type: "PDF", size: "184 KB", color: "bg-red-100 text-red-700" },
    { name: "Claim Form Template", type: "DOCX", size: "92 KB", color: "bg-blue-100 text-blue-700" },
    { name: "FCA Compliance Notice", type: "PDF", size: "156 KB", color: "bg-red-100 text-red-700" },
    { name: "Vehicle Inspection Report", type: "DOCX", size: "118 KB", color: "bg-blue-100 text-blue-700" },
  ];
  return (
    <MockFrame gradient="from-orange-400/30 via-primary/10 to-transparent">
      ...
      <div className="space-y-2">
        {docs.map(d => (
          <div key={d.name} className="rounded-lg border border-slate-200 bg-slate-50 p-3 flex items-center gap-3">
            <div className={`w-8 h-8 rounded-md flex items-center justify-center text-[9px] font-bold ${d.color}`}>
              {d.type}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium truncate text-slate-900">{d.name}</p>
...
export function SupportMock() {
  const tickets = [
    { ref: "TKT-104", subject: "How do I reset a customer password?", status: "Open", time: "2h ago", color: "bg-amber-100 text-amber-700" },
    { ref: "TKT-103", subject: "Bulk import warranties from CSV", status: "Resolved", time: "1d ago", color: "bg-emerald-100 text-emerald-700" },
    { ref: "TKT-102", subject: "Question about FCA exemption", status: "Resolved", time: "3d ago", color: "bg-emerald-100 text-emerald-700" },
  ];
  return (
    <MockFrame gradient="from-cyan-400/30 via-primary/10 to-transparent">
      ...
      <div className="space-y-2">
        {tickets.map(t => (
          <div key={t.ref} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs font-medium text-slate-900">{t.subject}</p>
          </div>
        ))}
      </div>
    </MockFrame>
